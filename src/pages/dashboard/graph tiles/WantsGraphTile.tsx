...
export default function wantsGraphTile(transactions: Transaction[], userPrefs: UserPrefs) {
    const data: {
        date: string,
        amount: number,
        goal: number
    ...
    for (const transaction of revTransactions) {
        ...
        while (true) {
            ...
            data.push(
                {
                    date: date,
                    amount: round(-currentWants, 2),
                    goal: round(currentIncome * userPrefs.getWantsBudget(), 2),
                }
            )
            currentDate += 8.64e7;
        }
        ...
        if (transaction.getCategoryCategory() == "Wants") {
